...
static void MaxAttrDefault(NodeAttrs& attrs) {
  ATTR_DEFAULT(attrs, "axis", "()");
  ATTR_DEFAULT(attrs, "keepdims", "false");
  ATTR_DEFAULT(attrs, "exclude", "false");
}

static void MaxForward(
    NodeAttrs const& attrs,
    std::vector<TypePtr>& inputs,
    std::vector<TypePtr>& outputs,
    std::vector<std::vector<NodeAssertions> >& nas) {
  ...
  reduce_func f = [](z3_expr& tmp, z3_expr value)->void {
    tmp = op_ite(tmp < value, value, tmp);
  };
...
Z3_REGISTER_OP(max)
  .set_num_inputs(1)
  .set_num_outputs(1)
  .set_attr_default(MaxAttrDefault)
  .set_forward(MaxForward)
  .set_infer_shape(MaxInferShape)
  .set_infer_precision(MaxInferPrecision);
}
}
